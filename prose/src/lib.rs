...
pub fn markdown() -> Result<String, BoxError> {
	let filename = "./example.md";

    let contents = fs::read_to_string(filename)?;

	match parser::parse_markdown(&contents[..]) {
		Ok( (_, m) ) => {
			Ok(translator::translate(m))
		},
		Err(_) => {
			Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "oh no we are fucked!")))
		}
	}
}
...
pub(self) mod parser {
	...
	fn parse_markdown_inline(i: &str) -> IResult<&str, MarkdownInline> {
		alt((
			map( parse_italics, |s: &str| MarkdownInline::Italic(s.to_string()) ),
			map( parse_inline_code, |s: &str| MarkdownInline::InlineCode(s.to_string()) ),
			map( parse_boldtext, |s: &str| MarkdownInline::Bold(s.to_string()) ),
...
pub(self) mod translator {
	...
	fn translate_list_elements(lines: Vec<MarkdownText>) -> String {
		lines.iter().map( |line|
			format!("<li>{}</li>", translate_text(line.to_vec()))
	...
	fn translate_line(text: MarkdownText) -> String {
		format!("<p>{}</p>", translate_text(text))
	}
    ...
    mod tests {
		...
		fn test_translate_boldtext() {
			assert_eq!(translate_boldtext(String::from("bold af")), String::from("<b>bold af</b>"));
		}

		#[test]
		fn test_translate_italic() {
			assert_eq!(translate_italic(String::from("italic af")), String::from("<i>italic af</i>"));
		}

		#[test]
		fn test_translate_inline_code() {
			assert_eq!(translate_inline_code(String::from("code af")), String::from("<code>code af</code>"));
		}

		#[test]
		fn test_translate_link() {
			assert_eq!(translate_link(String::from("click me!"), String::from("https://github.com")), String::from("<a href=\"https://github.com\">click me!</a>"));
		}

		#[test]
		fn test_translate_image() {
			assert_eq!(translate_image(String::from("alt text"), String::from("https://github.com")), String::from("<img src=\"https://github.com\" alt=\"alt text\" />"));
		}

		#[test]
		fn test_translate_text() {
			let x = translate_text(vec![
				MarkdownInline::Plaintext(String::from("Foobar is a Python library for dealing with word pluralization.")),
